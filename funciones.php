<?php
	...
	function obtenerMarcacionesHoy($local){
		$query="select nombre,apellidos,numeroDocumento,tipo,local,fechaHora,fecha, l.descLocal from empleado e
	...
	function registrarMarca($numeroDocumento,$local){
		$respuesta=array("codigo"=>0, "mensaje"=>"");
		$tipo=0;
		$codigoEmpleado="";
		$query="select e.codigo,m1.tipo Entrada,m2.tipo Salida from empleado e
						left join marcacion m1 on m1.codigoEmpleado=e.codigo and m1.tipo=1
						left join marcacion m2 on m2.codigoEmpleado=e.codigo and m2.tipo=2
		...
		if($tipo!=0)
		{
			...
			$query="INSERT INTO marcacion(codigoEmpleado,fecha, local, tipo) VALUES ( ".
			"'".$codigoEmpleado."',".
			"current_date(),".
			$local.",".
			$tipo.")";
